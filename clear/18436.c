...
void build(int* array, int node, int low, int high) {
    if (low == high) {
        if (array[low] % 2 == 0) st.tree[node].v2 = 1;
        else st.tree[node].v1 = 1;
    }
    else {
        int mid = low + (high - low) / 2;
        build(array, node * 2 + 1, low, mid);
        build(array, node * 2 + 2, mid + 1, high);
        st.tree[node].v1 = st.tree[node * 2 + 1].v1 + st.tree[node * 2 + 2].v1; 
        st.tree[node].v2 = st.tree[node * 2 + 1].v2 + st.tree[node * 2 + 2].v2; 
    }
}
...
Pair query(int node, int low, int high, int start, int end) {
    ...
    Pair rv = {left.v1 + right.v1, left.v2 + right.v2};
    return rv;
}
...
int main() {
    ...
    //     else if (command == 2) printf("%d\n", range_query(v1-1, v2-1).v2); 
    //     else printf("%d\n", range_query(v1-1, v2-1).v1); 
    // }
    // fclose(file);
