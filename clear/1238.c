/*
    title: 파티
    tag: graph, reverse graph, Dijkstra
*/ 
...
void dijkstra(int* graph, int start, int* dist) {
    for (int i = 1; i <= n; i++) {
        dist[i] = INF;
    }
    dist[start] = 0;
    ...
    while (minHeap->len > 0) {
        ...
        for (int v = 1; v <= n; v++) {
            if (dist[u.ver] + graph[(n+1)*u.ver + v] < dist[v]) {
                dist[v] = dist[u.ver] + graph[(n+1)*u.ver + v];
...
int main() {
    ...
    // fscanf(file, "%d %d %d", &n, &m, &x);
    // int size = (n+1) * (n+1);
    // int* arr = (int*) malloc(sizeof(int)*size);
    // int* rev_arr = (int*) malloc(sizeof(int)*size);
    ...
    // init
    int* to_X = (int*) malloc(sizeof(int)*(n+1));
    int* from_X = (int*) malloc(sizeof(int)*(n+1));


    // answer
    dijkstra(rev_arr, x, to_X);
    dijkstra(arr, x, from_X);
    int ans = 0;
    for (int i = 1; i <= n; i++) {
